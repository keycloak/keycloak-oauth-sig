...
| `config.override.yaml` | Optional environment-specific overrides for `config.yaml` (copy from `config.override.example.yaml`).                        |
...
- `keycloak.enable_preauth_code` / `keycloak.enable_credential_offer_create`: required on Keycloak **26.7+** for pre-authorized issuance and `create-credential-offer` (see `config.override.example.yaml`).
...
- [MIGRATION_26.7.md](docs/MIGRATION_26.7.md) Checklist for upgrading the local OID4VCI harness from Keycloak 26.6.x to **26.7.0** (feature flags, VC grants, offer API, metadata).
