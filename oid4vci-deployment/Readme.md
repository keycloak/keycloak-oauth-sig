...
> Prereqs: **bash** (scripts use `#!/usr/bin/env bash` and `export -f`; do not run them under zsh via `source`), Java 17+ (Java 21 for `import`), `openssl`, `keytool`, `jq`, `yq`, `docker`, and optionally `figlet` for pretty banners.
>
...
### Host-mode path (`setup` on the host)

Use this when you want Keycloak from a tarball/branch on the host (same project for install, setup, config, and test):

```bash
./keycloak-ssi.sh install          # once; points the global CLI at this checkout
keycloak-ssi setup -d              # download/unpack Keycloak, start DB + Keycloak in background
keycloak-ssi config                # via CLI container; talks to host Keycloak over HTTP
keycloak-ssi test preauth IdentityCredential
```

Foreground setup (Ctrl+C stops Keycloak): `keycloak-ssi setup` in one terminal, then `config` / `test` in another.
...
- **Shell**: Always invoke via `./keycloak-ssi.sh` / `keycloak-ssi` or `bash script.sh`. Helpers use bash `export -f`; sourcing under zsh fails.
- **Verify Keycloak health**: `curl -k -s ${KEYCLOAK_ADMIN_ADDR}/realms/master` inside `cmd_setup` waits up to two minutes. `config` / `test` use the same HTTP check (not process PID), so they work when Keycloak runs on the host and the CLI runs in Docker.
...
- **Root-owned `target/` after CLI use**: the Compose `cli` service may create files in `target/` as root. If host `setup` then fails with `Permission denied`, fix ownership:

```bash
docker run --rm -v "$PWD":/work alpine chown -R "$(id -u):$(id -g)" /work/target
# or: sudo chown -R "$USER:$USER" target
```

## Troubleshooting

- **CLI cannot find the project**: run commands from the repo root or reinstall via `./keycloak-ssi.sh install`. Mixing an old global install (another checkout under `~/.local/share/keycloak-ssi-deployment`) with `./keycloak-ssi.sh` from a different tree causes missing installs and false “Keycloak is not running” errors — keep one root.
- **`config` / `test` and host Keycloak**: always use `keycloak-ssi config` / `keycloak-ssi test` so the CLI container sets `KEYCLOAK_SSI_IN_CONTAINER=1` and receives `HOST_WORK_DIR`.
